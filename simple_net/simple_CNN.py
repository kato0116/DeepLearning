...
def fit(net,optimizer,criterion,epochs,train_loader,test_loader,history,device):
    net = net.to(device)
...
class CNN(nn.Module):
    """
    nn.Conv2d(チャネル数,フィルタ数,カーネルサイズ,stirde,padding)
    nn.MaxPool2d(カーネルサイズ,stride)
    """
...
print("device:",device)
net = CNN(output_size,hidden_size)
...
history = fit(net,optimizer,criterion,epochs,train_loader,test_loader,history,device)
